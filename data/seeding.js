...
function generateRestaurantHasCS(restaurantIds, cookingStyleIds) {
    const restaurantHasCookingStyles = restaurantIds
        .map(restaurantId => {
            ...
            const nbRestaurantCookingStyle = Math.min(faker.datatype.number(10), Math.ceil(cookingStyleIds.length / 3)) + 1;
            const cookingStyles = [];
...
(async () => {
    ...
    logger(`${insertedManagers.length} managers inserted`);
    ...
    logger(`${insertedCookingStyles.length} cookingStyles inserted`);
    ...
    logger(`${insertedCities.length} cities inserted`);
    ...
    logger(`${insertedRestaurants.length} movies inserted`);
